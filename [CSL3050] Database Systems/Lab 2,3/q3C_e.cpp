...
int main()
{
    ...
    if (mysql_real_connect(conn, "localhost", "dbms_demo", "dbms_demo1#","lab2_q3", 3306, NULL, 0)) {
        ...
        snprintf(query, 256, "(SELECT extract(Year from date_of_employment) as year,count(name) as num from t1 group by (select extract(Year from date_of_employment)) order by num DESC) limit 1;");
