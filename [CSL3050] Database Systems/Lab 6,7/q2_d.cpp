...
map<int,Bucket*> bmap;
    ...
    bool search(Row R){
        ...
        for(auto i:bmap[bucket_id]->v){
            auto j = i.second;
            if(j.Author_ID== R.Author_ID && j.Author_Name==R.Author_Name && j.Book==R.Book && j.Book_ID==R.Book_ID){
                return true;
            }
        }
        return false;
    ...
    bool insert(Row R){
        ...
        if(bmap[req_id]->v.size() < bucket_size){
            ...
            cout<<"Error : The bucket size is exceeded. Hence, the given n is invalid"<<endl;
            return 0;
        }
    }
...
int main(){
    ...
    Row R16("Ko_Ya_2418","Find_KY_2418","Kobi Yamada","Finding Muchness");
...
//     start = clock();
    ...
    cout<<"time taken by the query to get executed is "<<fixed<<time_taken<<setprecision(20);
    cout<<" sec"<<endl;
    cout<<xd<<endl;
}
