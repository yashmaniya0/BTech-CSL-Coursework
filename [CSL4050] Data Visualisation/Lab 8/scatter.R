  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxGroupInput("classes", "Select classes to show:",
                         ...
                         selected = c(1, 2, 3)),
      selectInput("class1_color", "Normal color:", choices = c("Light Green", "Blue", "Green", "Orange", "Purple", "Yellow"), selected = "Light Green"),
      selectInput("class2_color", "Hyper color:", choices = c("Red", "Blue", "Green", "Orange", "Purple", "Yellow"), selected = "Orange"),
      selectInput("class3_color", "Hypo color:", choices = c("Red", "Blue", "Green", "Orange", "Purple", "Grey"), selected = "Grey"),
...
server <- function(input, output) {
  ...
  output$scatter_plot <- renderPlotly({
    ...
    plot <- plot_ly(data = filtered_data, x = ~get(input$x_axis), y = ~get(input$y_axis), color = as.factor(filtered_data$class)) %>%
