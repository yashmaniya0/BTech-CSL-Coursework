...
int main()
{
    ...
    if (mysql_real_connect(conn, "localhost", "dbms_demo", "dbms_demo1#","lab2_q1", 3306, NULL, 0)) {

        cout<<"Connected Successfully!"<<endl;
   
        char tableName[256] = "numbers";
        char query[256];

        // snprintf(query, 256, "CREATE TABLE `%s` (`id` int NOT NULL PRIMARY KEY, `name` varchar(255), `email` varchar(255), `phone` varchar(255));",tableName);
        snprintf(query,256,"SELECT 1+2-3 + 7*3");
