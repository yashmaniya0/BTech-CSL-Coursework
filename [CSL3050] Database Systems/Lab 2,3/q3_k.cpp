...
int main()
{
    ...
    if (mysql_real_connect(conn, "localhost", "dbms_demo", "dbms_demo1#","lab2_q3", 3306, NULL, 0)) {
        ...
        snprintf(query,256,"SELECT * FROM `salesman` GROUP BY name HAVING count(distinct coverage_city)=(SELECT count(distinct coverage_city) from `salesman`)");
        // cout<<"yee"<<endl;
        ...
        if(n==0){
            cout<<"No such condition in the given database"<<endl;
        }
