#include <bits/stdc++.h>
#include "./linear_hashing_with_dll.cpp"
#include <chrono>
...
void queries(Directory &d, vector<Data> &entries, int initial_global_depth, int bucket_size)
{
    ...
    cout << endl;
    cout << "Initial Global Depth (n) : " << initial_global_depth << endl;
    cout << "Bucket size : " << bucket_size << endl;
    cout << "Time taken : " << (float)time_taken / 1000 << " ms\n\n";
    cout << "-------------------------------------------------------\n\n";
}
...
int main()
{
    ...
    initial_global_depth = 3;
    Directory dsmall(initial_global_depth, bucket_size);
    cout << "Initialized directory structure with \nBucket size : " << bucket_size << "\nInitial Global depth (n) : " << initial_global_depth << endl
         << endl;

    queries(dsmall, entries, initial_global_depth, bucket_size);


    initial_global_depth = 11;
    Directory dbig(initial_global_depth, bucket_size);
    cout << "Initialized directory structure with \nBucket size : " << bucket_size << "\nInitial Global depth (n) : " << initial_global_depth << endl
         << endl;
    
    queries(dbig, entries, initial_global_depth, bucket_size);
